...
class ExpGraphInfluenceFunction(Exp):
    def __init__(self, args):
        ...
        run_accuracy = np.empty((0))
        run_accuracy_unlearning = np.empty((0))
        run_recall = np.empty((0))
        run_recall_unlearning = np.empty((0))
        run_precision = np.empty((0))
        run_precision_unlearning = np.empty((0))
        ...
        for run in range(self.args['num_runs']):
            ...
            f1_score, accuracy, recall, precision = self.evaluate(run)
            run_f1 = np.append(run_f1, f1_score)
            run_accuracy = np.append(run_accuracy, accuracy)
            run_recall = np.append(run_recall, recall)
            run_precision = np.append(run_precision, precision)
            ...
            run_accuracy_unlearning = np.append(run_accuracy_unlearning, accuracy_unlearning)
            run_recall_unlearning = np.append(run_recall_unlearning, recall_unlearning)
            run_precision_unlearning = np.append(run_precision_unlearning, precision_unlearning)

        f1_score_avg = np.average(run_f1)
        f1_score_std = np.std(run_f1)
        accuracy_avg = np.average(run_accuracy)
        accuracy_std = np.std(run_accuracy)
        recall_avg = np.average(run_recall)
        recall_std = np.std(run_recall)
        precision_avg = np.average(run_precision)
        precision_std = np.std(run_precision)
        self.logger.info("f1_score: avg=%s, std=%s" % (f1_score_avg, f1_score_std))
        self.logger.info("accuracy: avg=%s, std=%s" % (accuracy_avg, accuracy_std))
        self.logger.info("recall: avg=%s, std=%s" % (recall_avg, recall_std))
        self.logger.info("precision: avg=%s, std=%s" % (precision_avg, precision_std))
        ...
        accuracy_unlearning_avg = np.average(run_accuracy_unlearning)
        accuracy_unlearning_std = np.std(run_accuracy_unlearning)
        recall_unlearning_avg = np.average(run_recall_unlearning)
        recall_unlearning_std = np.std(run_recall_unlearning)
        precision_unlearning_avg = np.average(run_precision_unlearning)
        precision_unlearning_std = np.std(run_precision_unlearning)
        ...
        self.logger.info("accuracy of GIF: avg=%s, std=%s" % (accuracy_unlearning_avg, accuracy_unlearning_std))
        self.logger.info("recall of GIF: avg=%s, std=%s" % (recall_unlearning_avg, recall_unlearning_std))
        self.logger.info("precision of GIF: avg=%s, std=%s" % (precision_unlearning_avg, precision_unlearning_std))
        self.logger.info("GIF unlearning time: avg=%s seconds" % np.average(unlearning_time_avg))
    ...
    def evaluate(self, run):
        ...
        y_pred = posterior.argmax(axis=1).cpu().numpy()
        ...
        self.logger.info("Final Test Accuracy: %s" % (accuracy,))
        self.logger.info("Final Test Recall: %s" % (recall,))
        self.logger.info("Final Test Precision: %s" % (precision,))
        return test_f1, accuracy, recall, precision
    ...
    def gif_approxi(self, res_tuple):
        ...
        y_true = self.data.y[self.data['test_mask']].cpu().numpy()
        y_pred = self.target_model.posterior().argmax(axis=1).cpu().numpy()

        accuracy = accuracy_score(y_true, y_pred)
        recall = recall_score(y_true, y_pred, average="micro")
        precision = precision_score(y_true, y_pred, average="micro")

        return time.time() - start_time, test_F1, accuracy, recall, precision
